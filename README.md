# Hi there visitor, I'm Francisco 👋

[![LinkedIn Badge](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/francisco-camacho-29b0b942/)
[![GitHub followers](https://img.shields.io/github/followers/pachocamacho1990?style=for-the-badge&logo=github)](https://github.com/pachocamacho1990?tab=followers)

## 🚀 About Me

Astrophysicist, startup founder, and Anthropic community ambassador in Colombia, with over a decade in Latin America's technology ecosystem. At Platzi he built the platform's first machine learning systems and led the data science team; as co-founder of Hunty he built an AI system that lets large enterprises automate their recruiting and selection processes intelligently and at scale, earning recognition from accelerator programs including Google for Startups and the AWS Generative AI Startup Program. Before his life in the startup ecosystem, he worked in consulting as a Google Cloud Certified Data Engineer, building machine learning solutions for major industries. As a scientist, he spent several years researching astrophysical plasmas and space turbulence, collaborating with leading supercomputing laboratories at NASA Ames and the University of São Paulo in Brazil. His trajectory reflects a continuous effort to unify the best of all his worlds — scientist, consultant, and startup founder — turning complex technical knowledge into tangible products that impact the lives of millions.

**Previous:** 
 - Co-Founder & CTO at [Hunty](https://hunty.com) - Scaled platform from MVP to ISO 27001-certified enterprise solution
 - CTO at [Humanolab](https://humanolab.ai) - Hands-on technical lead at an early stage startup. Personally built and shipped production code fast by leveraging AI tooling heavily across the stack, an AI-native development workflow that compressed delivery from weeks to days. 

**Current:**

- AI Builder School [website](https://www.builder-school.academy)
- Building in Stealth something awesome, stay tuned .... 

- 📫 How to reach me: [@el_pachocamacho](https://x.com/el_pachocamacho)
         
## 🔬 Research & Learning
         
- **Lablab.ai Hackaton Space:** [Solutions created with AI tools](https://lablab.ai/event/lablab-ai-hackaton-space)
- **Blog and sharing spaces:** [Academic space](https://www.youtube.com/@franciscocamacho1990) I opened some years ago for my students
